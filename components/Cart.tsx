...
const Cart = () => {
  ...
  return (
    <div
      ...
      }  bg-gray-100 w-80 shadow-2xl py-4 overflow-auto transition-all duration-700 ease-in-out z-50`}
    >
      <Button
        text='Close'
        action={() => toggleCart(!isOpen)}
        className='font-bold text-sm !w-[fit-content] px-3 ml-4'
      />
      {items.length > 0 ? (
        <div>
          <div className='my-4 flex flex-col gap-4 max-h-[calc(100vh-168px)] overflow-auto'>
            {items.map((item) => (
              <div
                key={item.id}
                className='flex items-center p-4 bg-white gap-3 rounded-md'
              >
                <div>
                  <Image
                    src={item.imageUrl}
        ...
        <p className='mt-2 ml-4'>Cart is empty!</p>
      )}
    </div>
  )
}
